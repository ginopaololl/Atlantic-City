document.addEventListener("DOMContentLoaded", () => {
  ...
  // Prioridad: Sistema auth.js (currentUser)
  const authUser = JSON.parse(localStorage.getItem("currentUser"));

  if (authUser && authUser.nombres) {
    currentUser = authUser.nombres;
  } else {
    // Fallback: Sistema chat local (userSession) - Por compatibilidad
  ...
  function loadChatHistory() {
    ...
    const currentMsgCount = chatBody.querySelectorAll(
      ".message:not(.initial)"
    ).length;
  ...
  function handleAutoResponse() {
    ...
    const lastTime =
      userMsgs.length > 1
        ? new Date(userMsgs[userMsgs.length - 2].timestamp)
        : new Date(0);
    ...
    if (history.length === 1 || minutesDiff > 30) {
      setTimeout(() => {
        const autoReply = {
          ...
          sender: "Sistema",
          role: "moderator",
          text: `Hola, recibimos tu consulta: "${limitText(
            lastUserMsg.text
          )}".<br>Un asesor se estará comunicando contigo en breve. Por favor espera un momento.`,
          timestamp: new Date().toISOString(),
        };
